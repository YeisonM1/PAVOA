...
const normalizeProductos = (productos) => productos.map((producto) => ({
  ...producto,
  variantesParseadas: parseVariantes(producto.variantes),
}));
...
export default function CategoriaPage() {
  ...
  const productosNormalizados = useMemo(() => normalizeProductos(productosDB), [productosDB]);

  const tallasDisponibles = useMemo(() => {
    const orden = ['XS', 'S', 'M', 'L', 'XL', 'XXL', 'ÚNICA'];
    const set = new Set();
    productosNormalizados.forEach((p) => {
      p.variantesParseadas.forEach((v) => {
  ...
  const productosFiltrados = useMemo(() => {
    let res = [...productosNormalizados];
    if (tallasFiltro.length > 0) {
      res = res.filter((p) => {
        const tallas = p.variantesParseadas.map(v => v.talla);
        return tallasFiltro.every(t => tallas.includes(t));
      });
    }
    if (coloresFiltro.length > 0) {
      res = res.filter((p) => {
        const colores = p.variantesParseadas.map(v => v.color);
    ...
    return res;
  }, [productosNormalizados, tallasFiltro, coloresFiltro, sortOption]);
